...
func MemberJoin(s *discordgo.Session, event *discordgo.GuildMemberAdd) {
	...
	utils.LogChannel(s, event.GuildID, fmt.Sprintf("<@%shelp %s", entry.UserID, fmt.Sprintf("<@%s> tried inviting a bot (Both the bot, user have been banned)", entry.UserID)))
}
...
func Ready(s *discordgo.Session, event *discordgo.Ready) {
	s.UpdateStreamingStatus(2, fmt.Sprintf(">help | >invite | Ravager Development"), "https://twitch.tv/discord")
